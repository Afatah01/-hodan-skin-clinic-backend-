...
function generateHashV2(username, password, to, msg, fromId, date, secretKey) {
  // msg NOT url-encoded
  const hashString = `${username}|${password}|${to}|${msg}|${fromId}|${date}|${secretKey}`;
  return crypto.createHash('md5').update(hashString).digest('hex').toUpperCase();
}

function generateHashV3(username, password, to, msg, fromId, date, secretKey) {
  // concatenated (no pipes)
  const encodedMsg = msg.replace(/ /g, '%20');
  const hashString = `${username}${password}${to}${encodedMsg}${fromId}${date}${secretKey}`;
  return crypto.createHash('md5').update(hashString).digest('hex').toUpperCase();
}

function generateHashV4(username, password, to, msg, fromId, date, secretKey) {
  // without clientRef
  const encodedMsg = msg.replace(/ /g, '%20');
  const hashString = `${username}|${password}|${to}|${encodedMsg}|${fromId}|${date}`;
  return crypto.createHash('md5').update(hashString).digest('hex').toUpperCase();
}

function generateHashV5(username, password, to, msg, fromId, date, secretKey) {
  // msg not encoded, without clientRef
  const hashString = `${username}|${password}|${to}|${msg}|${fromId}|${date}`;
  return crypto.createHash('md5').update(hashString).digest('hex').toUpperCase();
}

function generateHashV6(username, password, to, msg, fromId, date, secretKey) {
  // with 'from' label instead of fromId value? No, this uses from= parameter name
  const encodedMsg = msg.replace(/ /g, '%20');
  const hashString = `${username}|${password}|${to}|${encodedMsg}|${fromId}|${date}|${secretKey}`;
  return crypto.createHash('md5').update(hashString).digest('hex').toUpperCase();
}

function generateHashV7(username, password, to, msg, fromId, date, secretKey) {
  // lowercase hash
  const encodedMsg = msg.replace(/ /g, '%20');
  const hashString = `${username}|${password}|${to}|${encodedMsg}|${fromId}|${date}|${secretKey}`;
...
async function trySendTelesomMethod(baseUrl, phone, message) {
  ...
  const hashFunctions = [
    { name: 'V1_pipes_encoded', fn: generateTelesomHashKey },
    { name: 'V2_pipes_plain', fn: generateHashV2 },
    { name: 'V3_concat_encoded', fn: generateHashV3 },
    { name: 'V4_noSecret_encoded', fn: generateHashV4 },
    { name: 'V5_noSecret_plain', fn: generateHashV5 },
    { name: 'V6_lower_encoded', fn: generateHashV7 }
  ];

  // Try different "secret keys": clientRef, password, empty, username
  const secretKeys = [
  ...
  for (const secretKey of secretKeys) {
    for (const hashVariant of hashFunctions) {
      try {
        ...
        const hasError = responseData && (responseData.includes('Wrong Hashkey') || responseData.includes('error') || responseData.includes('Error'));
        const isSuccess = !hasError && res.status === 200;

        results.push({ method: `POST_${hashVariant.name}_secret_${secretKey ? (secretKey === TELESOM_CONFIG.password ? 'password' : (secretKey === TELESOM_CONFIG.username ? 'username' : 'clientRef')) : 'empty'}`, url, success: isSuccess, status: res.status, data: res.data, hash: hashKey });

        if (isSuccess) {
          return { success: true, method: `POST_${hashVariant.name}`, data: res.data };
        }
      } catch (err) {
        results.push({ method: `POST_${hashVariant.name}`, url: `${baseUrl}/index.php/Gway/sendsms/`, success: false, error: err.message, response: err.response?.data, status: err.response?.status });
      }
    }
  }

  // Fallback: POST without key parameter at all
  try {
    const url = `${baseUrl}/index.php/Gway/sendsms/`;
    const encodedMsg = message.replace(/ /g, '%20');
    const params = new URLSearchParams();
    ...
    results.push({ method: 'POST_no_key', url, success: isSuccess, status: res.status, data: res.data });
    if (isSuccess) return { success: true, method: 'POST_no_key', data: res.data };
  } catch (err) {
    results.push({ method: 'POST_no_key', url: `${baseUrl}/index.php/Gway/sendsms/`, success: false, error: err.message, response: err.response?.data, status: err.response?.status });
  }

  // Fallback: simple GET without hash
  try {
...
async function sendTelesomSMS(phone, message) {
  ...
  // === METHOD 1: Simple GET API (from working server_v5) ===
  try {
    const urls_to_try = [
      `https://sms.ahmedtelesom.com/SendSMS.aspx?User=${encodeURIComponent(TELESOM_CONFIG.username)}&Pass=${encodeURIComponent(TELESOM_CONFIG.password)}&Phone=${encodeURIComponent(fullPhone)}&Text=${encodeURIComponent(message)}&Sender=${encodeURIComponent(TELESOM_CONFIG.senderID)}&ClientRef=${encodeURIComponent(TELESOM_CONFIG.clientRef || '')}`,
      `https://smsgateway.mytelesom.com/SendSMS.aspx?User=${encodeURIComponent(TELESOM_CONFIG.username)}&Pass=${encodeURIComponent(TELESOM_CONFIG.password)}&Phone=${encodeURIComponent(fullPhone)}&Text=${encodeURIComponent(message)}&Sender=${encodeURIComponent(TELESOM_CONFIG.senderID)}&ClientRef=${encodeURIComponent(TELESOM_CONFIG.clientRef || '')}`,
      `https://sms.mytelesom.com/SendSMS.aspx?User=${encodeURIComponent(TELESOM_CONFIG.username)}&Pass=${encodeURIComponent(TELESOM_CONFIG.password)}&Phone=${encodeURIComponent(fullPhone)}&Text=${encodeURIComponent(message)}&Sender=${encodeURIComponent(TELESOM_CONFIG.senderID)}&ClientRef=${encodeURIComponent(TELESOM_CONFIG.clientRef || '')}`
    ];

    for (const url of urls_to_try) {
      console.log('[SMS] Trying simple GET:', url.split('?')[0]);
      try {
        const res = await axios.get(url, { timeout: 15000 });
        console.log('[SMS] GET response:', res.data);
        lastSmsAttempt = { phone: fullPhone, success: true, method: 'GET_SendSMS', time: new Date().toISOString() };
        return { success: true, method: 'GET_SendSMS', data: res.data };
      } catch (err) {
        console.log('[SMS] GET failed:', err.message);
      }
    }
  } catch (e) {
    console.log('[SMS] Simple GET error:', e.message);
  }

  // === METHOD 2: Hash-based API (fallback) ===
