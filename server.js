/**
 * Hodan Skin Clinic - Backend API (FIXED v2)
...
try {
  ...
  // Debug logging (safe - doesn't show full key)
  console.log('[Debug] FIREBASE_PROJECT_ID:', process.env.FIREBASE_PROJECT_ID ? 'SET' : 'MISSING');
  console.log('[Debug] FIREBASE_CLIENT_EMAIL:', process.env.FIREBASE_CLIENT_EMAIL ? 'SET' : 'MISSING');
  console.log('[Debug] FIREBASE_PRIVATE_KEY length:', rawKey.length);
  console.log('[Debug] FIREBASE_PRIVATE_KEY starts with:', rawKey.substring(0, 50));
  
  // Try multiple ways to format the key
  let formattedKey = rawKey;
  
  // Way 1: Remove surrounding quotes if present
  if (formattedKey.startsWith('"') && formattedKey.endsWith('"')) {
    formattedKey = formattedKey.slice(1, -1);
    console.log('[Debug] Removed surrounding quotes');
  }
  
  // Way 2: Replace literal \n with actual newlines
  if (formattedKey.includes('\\n')) {
    formattedKey = formattedKey.replace(/\\n/g, '\n');
    console.log('[Debug] Replaced \\n with newlines');
  }
  
  // Way 3: Replace literal \r with actual carriage returns
  if (formattedKey.includes('\\r')) {
    formattedKey = formattedKey.replace(/\\r/g, '\r');
  }
  
  // Way 4: Normalize all newline types
  formattedKey = formattedKey.replace(/\r\n/g, '\n').replace(/\r/g, '\n');
  
  console.log('[Debug] Formatted key length:', formattedKey.length);
  console.log('[Debug] Key has BEGIN:', formattedKey.includes('BEGIN PRIVATE KEY'));
  console.log('[Debug] Key has END:', formattedKey.includes('END PRIVATE KEY'));
  ...
  if (serviceAccount.projectId && serviceAccount.clientEmail && formattedKey.includes('BEGIN PRIVATE KEY')) {
    ...
    console.log('  - hasKey:', formattedKey.includes('BEGIN PRIVATE KEY'));
  }
} catch (err) {
